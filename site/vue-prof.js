...
(function () {
  ...
  function vueLecon(mid, ref, type) {
    ...
    N.chargerContenu(mid, ref).then((contenu) => {
        ...
        + `<div class="p-lecteur">
          ...
          <p class="p-eval-acces">${(() => { const d = N.decisionAcces(N.cle(mid, ref) + '/evaluation'); const a = N.etat.acces[N.cle(mid, ref) + '/evaluation']; return d === true
            ...
            : `<span class="p-etat p-etat-vide">évaluation fermée</span> <button type="button" class="p-bouton p-bouton-mini" data-eval-acces="ouvrir">Ouvrir l'évaluation pour sept jours</button>`; })()}</p>
              <div class="p-lecture-mode"><button type="button" class="p-bouton p-bouton-fantome p-bouton-mini" id="p-mode-lecture" aria-pressed="${N.lire('opaline.lecture.prof', 'page') === 'diapo'}">Lire en diapositives, comme elle</button></div>
              <div id="p-fiche-hote" hidden></div>
      ...
      // A28 : le même lecteur que Sterenn, corrigés visibles ; la page entière reste à un clic.
      const lecture = () => {
        const diapo = N.lire('opaline.lecture.prof', 'page') === 'diapo';
        const hote = document.getElementById('p-fiche-hote'); const article = vue().querySelector('.p-fiche'); const b = document.getElementById('p-mode-lecture');
        if (!hote || !article) return;
        b.setAttribute('aria-pressed', String(diapo));
        if (!diapo) { hote.hidden = true; hote.innerHTML = ''; article.hidden = false; return; }
        const sections = window.LECTEUR.decouper(doc.html);
        if (sections.length < 2) { hote.hidden = true; article.hidden = false; return; }
        article.hidden = true; hote.hidden = false;
        window.LECTEUR.monter(hote, { sections, position: 0, duree: doc.duree, libelleFin: 'Fin de la fiche', surTerminer: () => N.signaler('Fin de la fiche.', 'info'), surPage: () => { N.ecrire('opaline.lecture.prof', 'page'); lecture(); } });
      };
      document.getElementById('p-mode-lecture').addEventListener('click', () => { N.ecrire('opaline.lecture.prof', N.lire('opaline.lecture.prof', 'page') === 'diapo' ? 'page' : 'diapo'); lecture(); });
      lecture();
  ...
  function vueSterenn() {
    ...
    afficher(entete('Sterenn', 'Sa carte, ses règles, son point de départ, et ta carte à toi.')
      + '<div class="p-grille2"><div>'
      + (compagnon ? bloc('Son compagnon', compagnon) : '')
      + bloc('Sa carte', carte)
  ...
  /* ---------- Routage ------------------------------------------------------- */
  /* A27 : les pages secondaires vivent dans vue-prof-pages.js, chargé à la première visite. */
  let pagesSecondaires = null;
  function pages() {
    if (pagesSecondaires) return Promise.resolve(pagesSecondaires);
    return N.chargerScript('vue-prof-pages.js').then(() => { pagesSecondaires = window.VUE_PROF_PAGES({ DOMAINES, N, POIDS, afficher, bloc, brancherNiveaux, choixNiveau, entete, teinte }); return pagesSecondaires; });
  }
  function vueJournal(...a) { return pages().then((p) => p.vueJournal(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vueJeuxProf(...a) { return pages().then((p) => p.vueJeuxProf(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vueAideProf(...a) { return pages().then((p) => p.vueAideProf(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vueBulletin(...a) { return pages().then((p) => p.vueBulletin(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vueSocle(...a) { return pages().then((p) => p.vueSocle(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vuePeriodes(...a) { return pages().then((p) => p.vuePeriodes(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vueDocuments(...a) { return pages().then((p) => p.vueDocuments(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vueProgramme(...a) { return pages().then((p) => p.vueProgramme(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
  function vueRecherche(...a) { return pages().then((p) => p.vueRecherche(...a)).catch((e) => { N.signaler('Cette page n\'a pas pu être chargée : ' + e.message); }); }
