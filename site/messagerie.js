...
(function () {
  ...
  function reactionsHTML(m, moiRole, classe) {
    ...
    const puces = Object.keys(r).map((e) => {
      ...
      return `<button type="button" class="${classe}-puce ${mienne ? 'mienne' : ''}" data-reagir="${m.id}" data-emoji="${e}" title="${qui}" aria-label="${e} par ${qui}${mienne ? ', retirer' : ''}"><span>${e}</span>${r[e].length > 1 ? `<b>${r[e].length}</b>` : ''}</button>`;
    }).join('');
    return `<div class="${classe}">${puces}
      ...
      <div class="${classe}-menu" data-menu="${m.id}" hidden>${REACTIONS.map((e) => `<button type="button" data-reagir="${m.id}" data-emoji="${e}" aria-label="Réagir ${e}">${e}</button>`).join('')}</div>
    </div>`;
  }
  ...
  window.MESSAGERIE = { EMOJIS, REACTIONS, formater, filDe, barreFils, filtrer, reactionsHTML, brancherReactions, selecteurEmojis, inserer, barreFormatage, brancherFormatage, eclat };
})();
